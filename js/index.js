// toggle icon navbar 

let menuIcon = document.querySelector('#menu-icon');
let navbar = document.querySelector('.navbar');


menuIcon.onclick = () => { 
    menuIcon.classList.toggle('bx-x');
    navbar.classList.toggle('active');
}


//  scroll sections active link

let sections = document.querySelectorAll('section');
let navLinks = document.querySelectorAll('header nav a');

window.onscroll = () => {
    sections.forEach(sec => {
        let top = window.scrollY;
        let offset = sec.offsetTop - 150;
        let heigth = sec.offsetHeight;
        let id = sec.getAttribute('id');

        if (top >= offset && top < offset + heigth) {
            navLinks.forEach(links => {
                links.classList.remove('active');
                document.querySelector('header nav a [href*=' + id + ']').classList.add('active');
            });
        }
    });
//   sticky navbar

let header  = document.querySelector('header');

 header.classList.toggle('sticky', window.scrollY > 100 );


 menuIcon.classList.remove('bx-x');
 navbar.classList.remove('active');
}


// scroll reveal

ScrollReveal ({ 
    // reset: true,
    distance: "80px",
    duration: 2000,
    deley: 200
});

ScrollReveal().reveal('.home-content, .heading', { origin:"top"});
ScrollReveal().reveal('.home-img, .services-container, .portfolio-box, .contact form', { origin:
    "bottom"});


ScrollReveal().reveal('.home-content h1, .about-img', { origin:"left"});
ScrollReveal().reveal('.home-content p, .about-content', { origin:"rigth"});

// typed js  

const typed = new Typed('.multiple-text', {
strings: [' Desarrollador Frontend', 'Programador',],
typeSpeed:100,
backSpeed:100,
backDelay:1000,
loop: true
});
function mostrarContenido(event) {
    event.preventDefault(); // Evita el comportamiento predeterminado del enlace

    var botonLeerMas = event.target;
    var contenido = botonLeerMas.previousElementSibling; // Selecciona el elemento anterior al botón (el div .contenido-add)

    if (contenido.style.display === "none") {
        contenido.style.display = "block";
        botonLeerMas.innerText = "Leer menos";
    } else {
        contenido.style.display = "none";
        botonLeerMas.innerText = "Leer más";
    }
}

